    ...
    if num == "0":
        ...
        if currentPlayer.gamePlay.token_type == 't':
            ...
            if currentPlayer.gamePlay.token_type == 'c':
                ...
                token_string3 = '점수는'
        context['contents'] = '본 게임은 %d명이 한 조로 구성되며,<br> 모든 구성원에게는 매번 %d개의 %s 주어집니다.<br><br>' \
                              '당신은 %s의 %s을<br><b> 개인 펀드 또는 공공 펀드</b>에 넣을 것인지 선택할 수 있습니다.<br><br>' \
                              '<b>공공 펀드</b>에 투자하지 않은 %s<br> 자동적으로 <b>개인 펀드</b>에 적립됩니다.' \
                              % (currentPlayer.gamePlay.players, currentPlayer.gamePlay.token_given, token_string, token_count_string, token_string2, token_string3 )
        ...
        context['contents'] = '<b>개인 펀드</b>에 넣은 %s 1%s당 수익은 %s,<br> <b>공공 펀드</b>에 넣은 %s 1%s당 수익은 %s이며, <br>' \
                              '총 %d번의 시행이 끝난 후 당신의 총 수익으로 합산됩니다.' \
                              % (token_string,token_string_count,str(token_multiply_private),
                                 token_string, token_string_count,str(token_multiply_public) + token_string_count,
        ...
        context['contents'] = '예를 들어,<br> 당신이 개인 펀드에 투자하여 얻을 수 있는 수익은 다음과 같습니다.<br><br>' \
                              '1. 개인 펀드에 %s 55%s 가지고 있다면 : %s (55 x %d)<br>' \
                              '2. 개인 펀드에 %s 148%s 가지고 있다면 : %s (148 x %d)<br>' \
                              '3. 개인 펀드에 %s 0%s 가지고 있다면 : %s (0 x %d)'\
                              % (token_string, token_string_count, str(token_multiply_private * 55) + token_string_count, token_multiply_private,
                                 token_string, token_string_count, str(token_multiply_private * 148) + token_string_count,token_multiply_private,
        ...
        context['contents'] = '반면에, 공공 펀드에서 얻는 수익은 <br><b>' \
                              '"당신을 포함한 모든 구성원이 공공 펀드에 넣은 %s의 개수에 의해"</b>' \
                              '결정됩니다.<br>구성원들이 더 많이 투자할 수록 더 많은 이익을 얻으며,<br> 수익은 모든 구성원에게 똑같이 배분됩니다.' \
                              % (token_string )
        context['next_page'] = 'intro'
        context['next_index'] = int(num) + 1
        ...
        context['contents'] = '예를 들어 <br><br>' \
                              '1) 당신이 공공 펀드에 %s을 0%s 투자했으나,<br> 다른 구성원들이 투자한 %s이 총 100%s라면 : <br>모두 1인당 %s 씩 (100 x %d)<br><br>' \
                              '2) 당신이 공공 펀드에 %s을 40%s 투자하고,<br> 다른 구성원들이 투자한 %s이 총 80%s라면 : <br>모두 1인당 %s 씩 (120 x %d)<br><br>' \
                              '3) 당신이 공공 펀드이 %s을 60%s 투자하고,<br> 다른 구성원들이 투자한 %s이 총 0%s라면 : <br>모두 1인당 %s 씩 (60 x %d)' % \
                             ( token_string, token_string_count, token_string, token_string_count, str(100 * token_multiply_public), token_multiply_public,
                                token_string, token_string_count, token_string, token_string_count, str(120 * token_multiply_public), token_multiply_public,
                                token_string, token_string_count, token_string, token_string_count, str(60 * token_multiply_public), token_multiply_public )
        context['next_page'] = 'intro'
        context['next_index'] = int(num) + 1
        ...
        context['contents'] = '따라서 %d번의 게임이 끝난 후 당신의 총 수익은 다음과 같습니다. <br><br>' \
                              '총 수익 <br>= (<b>개인 펀드</b>의 총 %s x %d) + (<b>공공 펀드</b>의 총 %s x %d)' % \
...
def practice(request,num="0"):
    ...
    if num == "0":
        ...
        request.session['wait'] = currentPlayer.gamePlay.wait_time
        ...
        token_string = '토큰' if currentPlayer.gamePlay.token_type == 't' else '사이버머니' if currentPlayer.gamePlay.token_type == 'c' else '점수'
        token_string_count = '개' if currentPlayer.gamePlay.token_type == 't' else '원' if currentPlayer.gamePlay.token_type == 'c' else '점'

        request.session['iterate'] += 1
        template = 'game/input_money.html'
        context['next_page'] = 'practice'
        context['next_index'] = 2
        context['contents'] = '%s의 %s이 지급되었습니다.<br>%s의 %s을 공공펀드에 투자하겠습니까?' \
                              % ('10000'+token_string_count, token_string, '몇 '+token_string_count,token_string)
        context['question'] = ''
    elif num == "2":
        minus = int(request.POST.get('minus'))
        request.session['money'] = 10000
        fake_rand = random.randrange(0,10000) * currentPlayer.gamePlay.players
        request.session['total'] += (10000-minus) * token_multiply_private + (minus+fake_rand) * token_multiply_public

        context['contents'] = ''

        if currentPlayer.gamePlay.feedback_my_donate_public:
            context['contents'] += '나의 공공펀드 투자 : %d<br>' % (minus)
        if currentPlayer.gamePlay.feedback_other_donate_public:
            context['contents'] += '다른 구성원의 투자 : %d<br>' % (fake_rand)
        if currentPlayer.gamePlay.feedback_my_only:
            context['contents'] += '나의 개인펀드 수익 : %d원<br>' % ((10000-minus) * token_multiply_private)
        if currentPlayer.gamePlay.feedback_my_public:
            context['contents'] += '나의 공공펀드 수익 : %d원<br>' % ((minus+fake_rand) * token_multiply_public)
        if currentPlayer.gamePlay.feedback_my_total:
            context['contents'] += '나의 총 수익 : %d원<br>' % (request.session['total'])
...
def real_game(request,num="0"):
    ...
    if num == "0":
        ...
        request.session['total'] = 0
        context['contents'] = "자, 준비가 되었으면 '다음'버튼을 눌러 게임을 진행해주세요."
        context['next_page'] = 'real_game'
        context['next_index'] = 1
    elif num == "1":
        context['contents'] = "지금부터 게임을 시작하겠습니다.<br> 이제부터 당신을 포함한 "+str(currentPlayer.gamePlay.players)+"명이 게임에 함께 참여하게 됩니다.<br> 당신의 선택을 해주세요"
        context['next_page'] = 'real_game'
        context['next_index'] = 2
        ...
        if currentPlayer.gamePlay.feedback_other_donate_public:
            context['contents'] += '다른 구성원의 투자 : %d<br>' % (fake_rand)
        if currentPlayer.gamePlay.feedback_my_only:
            context['contents'] += '나의 개인펀드 수익 : %d원<br>' % ((currentPlayer.gamePlay.token_given-minus) * token_multiply_private)
        if currentPlayer.gamePlay.feedback_my_public:
            context['contents'] += '나의 공공펀드 수익 : %d원<br>' % ((minus+fake_rand) * token_multiply_public)
        if currentPlayer.gamePlay.feedback_my_total:
            context['contents'] += '나의 총 수익 : %d원<br>' % (request.session['total'])
